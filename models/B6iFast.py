...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.models = nn.ModuleList([iTransformer.Model(configs).float().cuda() for i in range(self.num_models)])

        print("[CREATE] " +str(self.num_models)+" learners of iTransformer")
...
# 
    ...
    def forward_1learner(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        if self.output_attention:
            i = idx     
            do, attn = self.models[i].forward(x_enc, x_mark_enc, x_dec, x_mark_dec)
            return do, attn
        else:
            i = idx
            do = self.models[i].forward(x_enc, x_mark_enc, x_dec, x_mark_dec)
